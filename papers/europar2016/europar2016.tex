...
a processor that want to access its local memory node will have a better bandwidth
than if he wanted to access a non-local memory node.
Controlling data locality over the application lifetime is one of the key to
achieve good performances on these architectures.
...
needs which data : thanks to these dependencies it knows which data blocks are read,
written, or both, by which task.
The runtime has a lot of freedom in its scheduling strategies, and this work aims
at using both the data placement information and the topology of the architecture
to overall improve the data locality and tasks placement, in order to achieve better performances.

We distinguished three key points in the scheduling policy, this paper
will describe them, and will show how we can leverage on them to influence the
overall performances of an application, positively or negatively.
We evaluated our work on a large NUMA machine, and compared it to previously described 
approaches.
