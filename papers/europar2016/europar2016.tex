...
\author{
  Philippe Virouleau$^{3,2,1}$ \and François Broquedis$^4$ \and Thierry Gautier$^{5,3}$
 \vspace*{-1ex}}
\institute{
   $^1$Univ. Grenoble Alpes, LIG, Grenoble, France,
   $^2$CNRS, LIG, Grenoble, France,
   $^3$INRIA,
...
Because of the scalability issue of SMP, most Many-Core processors have a topology with non-uniform memory access (NUMA). If heavily used
nowadays, such architectures are not yet fully exploited to the best of their performances.
On such designs the memory is distributed among several nodes, and
both bandwidth and latency depends on which processor wants to access specific data~:
a CPU accessing its local (within the same node) memory will have a better bandwidth
than if accessing a remote memory.
Controlling data locality over the application lifetime is one of the key challenge to
achieve good performances on those architectures.

The recent extension of the OpenMP tasking model to allow dependent tasks, enables the
run-time to have very precise information about which part of an application
needs which data~: thanks to these dependencies, it is known which data blocks are read,
written, or both, and by which task.
The run-time has a lot of freedom in its scheduling strategies. This work aims
at using the data placement information and the topology of the architecture
to improve both data locality and tasks placement, thus achieving better performances.

We distinguish three key points in the scheduling policy, that we will describe later.
We will show how we can leverage on them to influence the
overall performances of an application, positively or negatively.
We evaluate our work on a large NUMA machine, and compared it to previously described 
approaches.

The layout of this paper falls into six sections as follow:
...
We  then describe in Section~\ref{sec:contributions} the ideas, strategies and implementation details that we used to improve the run-time performances
...
implemented with OpenMP task dependencies. It inspired by the SparseLU kernel from the BOTS~\cite{BOTS} benchmark suite.
...
In this work we make experimentations with KASTORS benchamrk suite~\cite{virouleau:hal-01081974} for OpenMP tasks with dependencies.
Factorization algorithms comes from PLASMA library are the structure is closed to algorithm presented in listing~\ref{lulst}.
...
Figure \ref{fig:eval-strat-sizes} reports their performance using a \verb/cyclicnuma/ distribution without steal restriction. 
