...
Thanks to Cilk~\cite{FrigoLeisersonRandall1998,CilkSched98}, the work stealing
technique has become popular and is often considered when it comes to
dynamically balance the work load among processing units. The work stealing
principle can be summarized as follows. An idle thread, called a thief,
initiates a steal request to a random selected victim. On reply, the thief
receives a copy of one ready task, leaving the original task marked as stolen.
Coherency between a thief and its victim is ensured by a variant of the  Cilk's T.H.E
protocol~\cite{FrigoLeisersonRandall1998}.

The \kaapi runtime system also implements the work-stealing execution model to execute OpenMP task-based applications.
The runtime creates a system thread, called a \emph{kproc}, for each computation resource to be used.
On a NUMA multicore machine, a resource is a core.
A kproc creates tasks and pushes them on its own work queue, which is implemented as a stack.
The enqueue operation is very fast : it takes around ten cycles on recent x86/64 processors~\cite{libkomp}.
...
In addition, \kaapi runtime allows the cooperation among thieves, i.e.,
multiple steal requests to the same processor with reduction of steal
requests~\cite{marc_paperWS}.
Previously, when $k > 1$ steal requests were sent on the same processor, only one request could be served.
If the victim kproc is in a stealing state, the thief aborts the steal request and selects another
victim randomly.
The \kaapi cooperation divides the work into $k+1$ pieces when $k$ steal
requests target the same processor.
...
of the machine, and is very similar to the notion of \verb/shepherd/ introduced
in \cite{DBLP:journals/ijhpca/OlivierPWSP12}.
