...
\title{Analysis and improvements of runtime strategies on NUMA architectures for task-based applications}
\author{
...
\begin{abstract}
  ...
  Using OpenMP 4.0 data dependencies for task-based applications enable the
  programmer to pass significant information to the underlying runtime system.
  ...
  NUMA architectures.  
  Most widely used OpenMP runtimes use workstealing to schedule tasks-based programs,
  and several strategies exist to improve their execution on NUMA architectures.
  Control over the data initialization and locality between a task and its data
  has been proven to be very efficient.
  In this paper we will make a comprehensive analysis of several applications'
  behavior on various NUMA architectures, with different number of cores, cache sizes,
  and NUMA factors.
  We will show how we extended the Intel OpenMP runtime with original NUMA features,
  and we will study the impact of each of them separately, comparatively to previous
  works.
\smallskip
...
When a processor becomes idle the runtime system has to \textit{select a victim} and calls a function, called  \verb/WSselect/ for \emph{work-stealing select}, to browse the topology to find a place from which stealing a task from the place task queue.
...
Before parallel computation begins, the runtime system can distribute (\textit{push}) the set of ready tasks to multiple places, according to the strategy defined by \verb/WSpush_init/.

These \textbf{three functions} are the main entry points to specify a \textbf{scheduling algorithm} in \kaapi.
Sections~\ref{sec:ws_push_init} to~\ref{sec:ws_select} describe strategies for
these three points, all them were designed to explore the possibilities of the target NUMA architectures,
to be able to evaluate which one are worth taking into account.
...
git repository\footnote{https://scm.gforge.inria.fr/anonscm/git/kaapi/xkaapi.git} in the branch \emph{public/europar2016}.
...
anyone may reproduce our data analysis, and look at the other results we did not put in the figures.
...
\begin{figure}[t]
  ...
  \caption{Evaluating all strategies (WSselect + WSpush), using \emph{cyclicnuma} WSpush\_init}
...
\begin{figure}[t]
  ...
  \caption{Evaluating specific strategies on multiple sizes, using \emph{cyclicnuma} WSpush\_init}
...
Finding the appropriate combination is highly application-dependent, therefore it is hard
to give a solution for every type of application. However some general guidelines can be followed:
\begin{itemize}
  \item The most critical part is the initial data distribution.
    Figure~\ref{fig:eval-distrib} showed it is absolutely necessary to use one.
  \item Hierarchical strategies have a cost, so the problem size has to generate
    enough work and data transfer to see a benefit.
\end{itemize}
...
Drebes et al.~\cite{Drebes:2014:TDS:2658949.2641764} proposed similar ideas in another
dataflow programming model named OpenStream. This model has a focus on data streaming
and has a lot of flexibility on their placement, but does not provide flexibility to the user.

While the same kind of studies have been conducted in other contexts~\cite{DBLP:conf/europar/TerbovenSCM12,DBLP:journals/corr/abs-1101-0093,Weng:2002:IOU:645610.661369}, none of them takes advantage of the OpenMP \verb/depend/ clause, which precisely indicates which data are read and written by a given task. As advertised by the results obtained by our \verb!sNumaProc+pNumaWLoc! combined strategy, this information is worth taking into account when choosing a place to push ready tasks to.
