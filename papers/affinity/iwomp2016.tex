...
The task can request to be executed close to:
\begin{itemize}
    \item A physical core.
    \item A NUMA node (by one of the cores on the node)
    \item A data (the task want to be executed on the node where the data is physically allocated)
\end{itemize}
...
The proposed syntax for the clause is the following:
...
TODO continuer la description de ce que j'ai fait dans le compilo

\subsection{Runtime extension}

TODO décrire les extensions côté runtime, une description de la vision de la "machine" depuis kaapi.

Parler de comment on a implémenté le "omp\_get\_numa\_from\_data".
...
TODO: j'ai oublié de faire tourner la version "non-strict" de task-affinity !
