...
Leur premier axe est de restreindre le vol de travail à l'intérieur du nœud : les threads ne peuvent voler que dans une file commune au nœud NUMA.
Le second est de tracer la localité des données : ils utilisent des tâches d'initialisations pour détecter où sont allouées les données. Lors de l'exécution du programme cette information est utilisée pour ordonnancer une tâche sur le nœud où il y a le plus de données manipulées par la tâche.
