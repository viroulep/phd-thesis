...
Il y a en général 3 niveaux de caches dans ces architectures :
\begin{itemize}
  ...
  \item L3 : Ce cache est généralement le cache de dernier niveau - \emph{Last Level Cache (LLC)} - sur un processeur. La latence pour y accéder est plus importante que le L2, mais sa capacité est bien supérieure, atteignant en général plusieurs Mo.
\end{itemize}

En fonction des fabricants, ces caches peuvent être \emph{inclusifs} (c'est-à-dire que toutes les données présentes dans le L1 sont également présentes dans le L2), ou \emph{exclusifs} (c'est-à-dire qu'une donnée n'est garantie d'être présente que dans l'un des caches).
...
Comme on peut le voir sur ces chiffres, le coût d'accès à la mémoire principale est bien plus grand que l'accès des différents caches, et l'accès à la mémoire distante est tout simplement prohibitif.
...
\begin{itemize}
  \item Random : cette politique choisit une ligne à remplacer au hasard. Sa simplicité la rend facile à implémenter en pratique, il a notamment été utilisée dans les processeurs ARM Cortex-R~\cite{ARM-Cortex-R}.
  ...
  \item Pseudo-LRU : cette politique offre une version dégradée permettant de gérer des caches de grande taille.
...
Cela permet de limiter le nombre de bits nécessaire pour tracer les potentielles lignes à remplacer.
...
Dans les processeurs Intel, des variantes de la politique pseudo-LRU semble être la plus utilisée pour les caches de faibles associativités, et donc typiquement les caches de niveaux L1 et L2~\cite{Abel2014,IntelXeonPhi2014}. Al-Zoubi et al.~\cite{Al-Zoubi2004} proposent une évaluation complète et détaillée de l'impact des politiques d'éviction en fonction de l'associativité du cache, justifiant le choix des constructeurs.
