...
Dans l'exemple du listing~\ref{lst:context:task-dep}, la tâche |read_A| possède une dépendance en \emph{lecture} sur la variable |a|, et la tâche |write_A| possède une dépendance en \emph{écriture}

\begin{todo}
  finir
\end{todo}
ajouter des dépendances de données, induisant un ordre sur l'exécution des tâches. On  Dans le listing~\ref{lst:context:task-dep}, la tâche C sera également garantie d'être exécutée \textbf{après} les tâches A et B, car elle consomme des données que A et B produisent.
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=\textwidth]{tasks_size_overhead}
